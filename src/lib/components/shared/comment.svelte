...
<comment class="flex gap-3 md:gap-[1vw]">
	<a href="/user/" class="h-7 w-7 flex-shrink-0 md:h-[2vw] md:w-[2vw]">
		<ImageLoader
	...
	<comment-details class="flex flex-col items-start gap-1 md:gap-0">
		<a href="/user/" class="text-xs leading-none md:text-[1vw]">
			<username>{comment_username}</username>
			<comment-time class="text-surface-300 md:text-[0.75vw] md:leading-[1.5vw]"
				>{new FormatDate(comment_date).format_to_time_from_now}</comment-time
			>
		</a>
		...
		{#if Array.isArray(comment_replies) && comment_replies.length}
			<replies-section class="md:mt-[0.35vw]">
				<Accordion padding="p-0" hover="bg-transparent">
					<AccordionItem
						{open}
								...
								<reply class="flex gap-3 md:gap-[1vw]">
									...
									<reply-details class="flex flex-col items-start gap-1 md:gap-0">
										<a href="/user/" class="text-xs leading-none md:text-[1vw]">
											...
											<reply-time class="text-surface-300 md:text-[0.75vw] md:leading-[1.5vw]"
												>{new FormatDate(reply.date).format_to_time_from_now}</reply-time
											>
										</a>
