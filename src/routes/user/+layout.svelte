...
<root class="relative grid h-screen w-screen md:grid-cols-2">
	{#if image && name}
		{#key image}
			...
			>
				<ImageLoader src={image} class="absolute h-full w-full object-cover" />
				...
				<div
					class="absolute inset-0 bottom-[6vw] hidden flex-col items-center justify-center text-center md:flex"
				>
					<span class="text-[0.75vw] font-semibold uppercase leading-none text-surface-50"
						>welcome to</span
					>
					...
					<span class="mt-[4vw] text-[0.9vw] font-semibold leading-none"
						>With a coreproject account, you can</span
					>
					<span
						class="mt-[1vw] max-w-[20.375vw] text-[0.9vw] font-medium leading-[1vw] text-surface-200"
				...
				<div class="absolute bottom-[1.85vw] left-10 md:left-[2vw] md:flex">
					<div class="flex flex-col gap-2 md:gap-[0.75vw]">
						<span
							class="text-[2.25vw] font-semibold uppercase leading-none tracking-widest text-surface-300/75 md:text-[0.75vw]"
							>Background from anime</span
						>
						<div class="flex items-center gap-[2vw] md:gap-[0.5vw]">
							<span
								class="text-[3vw] font-bold uppercase leading-none tracking-widest text-warning-400 md:text-[1vw]"
							>
								{name}
							</span>
	...
	{/if}
	<div
		class="z-0 col-start-1 col-end-1 row-start-1 row-end-1 flex items-center justify-center md:col-start-2 md:col-end-2 md:block md:items-end md:px-[8vw] md:py-[2.2vw]"
	>
		<slot />
	</div>
</root>
