...
{#if mapping}
	<login-container
		...
		class={cn(
			mapping.class,
			'flex h-full w-full grid-cols-12 flex-col justify-end md:grid md:items-start md:gap-[5vw] md:pl-[3.75vw]'
		)}
	>
		<form
			use:form
		...
		>
			...
			<span class="text-xl text-surface-50 md:text-[1vw] md:leading-[2vw]"
				>for seamless integration</span
			>
			...
			<providers class="mt-10 md:mt-[5vw]">
				<streamsb class="flex flex-col md:gap-[0.25vw]">
					...
					<ValidationMessage for="streamsb" let:messages={message}>
						...
						>
							Insert your unique API token here to unlock the full potential of Streamsb's video
							services
						</span>
					</ValidationMessage>
				</streamsb>
		...
		<character-image
			...
			style="--mobile-gradient:{mapping.gradient.mobile}; --desktop-gradient:{mapping.gradient
				.desktop}"
		>
			<gradient
				class={cn(
					mapping.gradient.class,
					'pointer-events-none absolute [background:var(--mobile-gradient)] md:[background:var(--desktop-gradient)]'
				)}
			/>
			...
			<img
				...
				class={cn(
					mapping.image.class,
					'relative h-[40dvh] object-contain object-bottom md:h-[90dvh]'
				)}
			/>
		</character-image>
	</login-container>
